...
( function( $, api ) {
    ...
    CustomifyAutoCSS.prototype.columns_settings = function ( field ) {
        ...
        if ( typeof values === 'string' && values.length ) {
            try { values = JSON.parse( decodeURIComponent( values ) ); }
            catch ( _ ) {
                try { values = JSON.parse( values ); } catch ( __ ) {}
            }
        }
        if ( ! _.isObject( values ) || _.isArray( values ) ) {
            values = {};
        ...
        var colSelectors      = _.isObject( field.col_selectors ) && ! _.isArray( field.col_selectors )
            ? field.col_selectors : {};
        ...
        if ( field.col_layout_setting ) {
            ...
            if ( _.isObject( clRaw ) && ! _.isArray( clRaw ) ) {
                ...
                try { clData = JSON.parse( decodeURIComponent( clRaw ) ); }
                catch ( e ) { try { clData = JSON.parse( clRaw ); } catch ( e2 ) {} }
            }
        ...
        _.each( deviceMap, function ( cssBucket, deviceKey ) {
            ...
            _.each( deviceValues, function ( colValue, colKey ) {
                ...
                var justifyValue = '';
                switch ( align ) {
                    case 'flex-start':    justifyValue = 'flex-start';    break;
                    case 'flex-center':   justifyValue = 'center';        break;
                    case 'flex-end':      justifyValue = 'flex-end';      break;
                    case 'space-between': justifyValue = 'space-between'; break;
                }
