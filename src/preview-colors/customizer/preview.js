...
(function () {
	...
	var DARK_BASELINES   = cfg.darkBaselines || { scss: {}, hex: {} };
	...
	function deriveDarkSlot(slot, srcHex, palette) {
		...
		switch (slot) {
			case 'base':      l = clamp(100 - l, 5, 12); break;
			case 'text':      l = clamp(100 - l, 88, 96); s = Math.min(s, 30); break;
			case 'surface':
				var baseSrc = palette && palette.colors && palette.colors.base;
				var baseDark = baseSrc ? deriveDarkSlot('base', baseSrc, palette) : '#0B0D10';
				var bRgb = hexToRgbArray(baseDark), bHsl = rgbToHsl(bRgb);
				h = bHsl[0]; s = bHsl[1]; l = Math.min(bHsl[2] + 6, 18);
				break;
			case 'primary':   l = clamp(l, 55, 70); break;
			case 'secondary':
				var surfHex = (palette && palette.colors && palette.colors.surface) || '#FFFFFF';
				var surfRgb = hexToRgbArray(surfHex);
				var lumSrc = luminance(rgb), lumSurf = surfRgb ? luminance(surfRgb) : 1;
				if (lumSrc < lumSurf) { l = clamp(100 - l, 80, 95); s = Math.max(s - 10, 0); }
				else { l = Math.max(l - 10, 20); }
				break;
			case 'accent':    s = Math.min(s + 10, 95); l = clamp(l, 55, 80); break;
		}
