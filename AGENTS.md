...
Edit `src/`, never `build/`. SCSS / JS sources live in `src/`. The `build/` folder is generated by webpack and committed for deploy convenience. The `assets/` folder is legacy and is no longer updated.
