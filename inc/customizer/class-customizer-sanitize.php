...
class Customify_Sanitize_Input {
	...
	private function sanitize_icon( $value ) {
		...
		// For `type === 'svg'` the icon is a preset-library KEY, not a CSS
		// class list — pin it to the key charset so a stray class string
		// can't be stored where a key is expected. Every other type keeps
		// sanitize_text_field(), which is what shipped, so no font-icon
		// class (`fa fa-shopping-basket` — spaces and all) is affected.
		$value['icon'] = ( 'svg' === $value['type'] )
			? sanitize_key( $value['icon'] )
			: sanitize_text_field( $value['icon'] );
