...
class Customify_Customizer_Control_Base extends WP_Customize_Control {
	...
	/**
	 * Suggested preset SVG icons for an `icon` field.
	 *
	 * A list of keys from the preset library (`customify_get_svg_icons()`),
	 * e.g. `array( 'cart', 'bag', 'basket' )`. When set, the icon picker
	 * shows those glyphs as a one-click "Suggested" row above the library
	 * browser. Optional everywhere — an icon field without it behaves
	 * exactly as it did before the row existed.
	 *
	 * @since 0.4.25
	 * @var array
	 */
	...
	public function to_json() {
		...
		// Always an array in JS, so the template can do `field.presets.length`
		// without a type guard on every icon control that omits the arg.
		$this->json['presets']          = is_array( $this->presets ) ? array_values( $this->presets ) : array();
	...
	\$(document).on('click', '.focus-control', function(e){
		...
		if ( is_null( self::$_args_loaded ) ) {
			...
			$args = array(
				...
				// Preset inline-SVG library for the icon picker: the grid,
				// the per-field "Suggested" row and the control's preview
				// chip all read this one map, so a Pro/child-theme icon
				// added through `customify/svg_icons` shows up in all three
				// with no extra registration. Markup is generated by PHP
				// (customify_get_svg_icon()) so the picker previews exactly
				// what the front end will print.
				'svg_icons'        => function_exists( 'customify_get_svg_icons_for_js' ) ? customify_get_svg_icons_for_js() : array(),
				'svg_icons_label'  => __( 'SVG Icons', 'customify' ),
				'suggested_label'  => __( 'Suggested', 'customify' ),
