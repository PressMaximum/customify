	...
	function customify_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . __( '<span class="cat-label">Posted in</span> %1$s', 'customify' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . __( '<span class="tagged-label">Tagged</span> %1$s', 'customify' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
